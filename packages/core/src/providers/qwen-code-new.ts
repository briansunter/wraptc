...
export class QwenCodeProvider extends ProcessProvider {
  constructor(config: ProviderConfig) {
    super("qwen-code", "Qwen Code CLI", config);
  }
  ...
  async runOnce(
    ...
    if (result.exitCode !== 0) {
      throw new Error(`Qwen Code CLI failed with code ${result.exitCode}: ${result.stderr}`);
    }
  ...
  classifyError(error: ProviderErrorContext): ProviderErrorKind {
    ...
    if (combined.includes("network") || combined.includes("timeout") || combined.includes("econn")) {
      return "TRANSIENT";
    }
    return "TRANSIENT";
  }
